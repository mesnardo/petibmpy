"""Implementation of the class `CartesianStructuredMesh` and sub-classes."""
...
class Segment(object):
    """Contain information about a segment."""

    def __init__(self, data=None, vertices=None, mode='cuibm'):
        """Create the segment vertices.

        Parameters
        ----------
        data : dictionary, optional
            YAML data; default: None.
        vertices : 1D array of floats, optional
            vertices along the segment; default: None.
        mode : string, optional
            Mode to use to create the segment;
            choices: ['cuibm']; default: 'cuibm'.
    ...
    def create_from_yaml_data(self, data, mode=None):
        """Create vertices from provided YAML data.
    ...
    def print_parameters(self):
        """Print the parameters of the segment."""
    ...
    def get_vertices(self, reverse=False, mode=None):
        """Compute the vertices of the segment.
        ...
        if abs(ratio - 1.0) < 1.0E-06:
            ...
            if abs(int(round(length / width)) - length / width) > 1e-12:
                print('[error] uniform discretization: '
                      'length of the segment should be a multiple '
        ...
        if mode == 'cuibm':
            ...
            return numpy.insert(self.start + numpy.cumsum(widths),
                                0, self.start)
        ...
        if reverse:
            ...
            return numpy.insert(self.end - numpy.cumsum(widths),
                                0, self.end)[::-1]
        else:
            return numpy.insert(self.start + numpy.cumsum(widths),
                                0, self.start)
    ...
    def get_stretch_ratio(self, width,
        ...
        """Compute the optimal stretching ratio.

        Compute the optimal stretching ratio given a targeted stretching ratio
        ...
        stretch_ratio : float, optional
            Targeted stretching ratio; default: 1.0.
    ...
    def compute_stretch_ratio(self, width, aspect_ratio,
                              precision=6):
        """Compute the stretching ratio provided the a targeted aspect ratio.

        The aspect ratio is given as the ratio between the first and
        last divisions of the segments.
        ...
        precision : integer, optional
            Precision of the stretching ratio to compute; default: 6.
    ...
    def compute_optimal_stretch_ratio(self, width, ratio, precision=6):
        """Compute the optimal stretching ratio provided a targeted one.
    ...
    def generate_yaml_info(self):
        """Generate a dictionary with segment's information ready for YAML.
...
class GridLine(object):
    """Contain information about a gridline."""

    def __init__(self, data=None, vertices=None, label=None, mode=None):
        """Create a gridline from provided YAML data or vertices.

        Parameters
        ----------
        data : dictionary, optional
            YAML data about the gridline; default: None.
        vertices : 1D array of floats, optional
            Vertices along the gridline; default: None.
        label : string, optional
            Label of the direction; default: None.
        mode : string, optional
            Mode to use to create the gridline;
            choices: ['cuibm']; default: None.
        ...
        if data:
            ...
            self.create_from_vertices(vertices)
        self.nb_divisions = sum(segment.nb_divisions
                                for segment in self.segments)

    def create_from_vertices(self, vertices):
        """Define the gridline from provided vertices as a single segment.
    ...
    def create_from_yaml_data(self, data, mode=None):
        """Initialize the gridline parameters and computes its vertices.

        A gridline is defined as a sequence of uniform and/or
        stretched segments.
    ...
    def get_vertices(self, precision=6):
        ...
        The method removes the repeated values at boundaries
        between consecutive segments.

        Parameters
        ----------
        precision : integer, optional
            Precision used to round the vertices
            so we can remove repeated values; default: 6.
    ...
    def print_parameters(self):
        """Print parameters of the gridline."""
    ...
    def generate_yaml_info(self):
        """Generate a dictionary with gridline's information ready for YAML.

        The dictionary contains the direction, the start, and segments
        information of the gridline.
...
class CartesianStructuredMesh(object):
    """Contain info related to a Cartesian structured mesh."""
    ...
    def get_number_cells(self):
        """Get the number of divisions and total number of cells."""
        nb_divisions = []
    ...
    def print_parameters(self):
        """Print parameters of the Cartesian structured mesh."""
        print('[info] printing parameters of the Cartesian mesh ...')
    ...
    def write(self, file_path, precision=6, direction='all'):
        """Write the gridlines into a file.
        ...
        with open(file_path, 'w') as outfile:
            if direction == 'all':
                ...
                outfile.write('\t'.join(str(nb)
                              for nb in nb_cells_directions) + '\n')
        with open(file_path, 'ab') as outfile:
            p = precision
            if direction in ['x', 'all']:
                numpy.savetxt(outfile,
                              self.gridlines[0].get_vertices(precision=p))
            if direction in ['y', 'all']:
                numpy.savetxt(outfile,
                              self.gridlines[1].get_vertices(precision=p))
            if len(self.gridlines) == 3 and direction in ['z', 'all']:
                numpy.savetxt(outfile,
                              self.gridlines[2].get_vertices(precision=p))
    ...
    def write_yaml_file(self, file_path):
        """Write a YAML file with information about the Cartesian mesh.
        ...
        with open(file_path, 'w') as outfile:
            ...
            outfile.write(yaml.dump({'mesh': data}, default_flow_style=False))
